...
function isInsideTranslation(node: ts.Node): boolean {
  ...
  while (p) {
    ...
    if (ts.isJsxExpression(p) && p.expression && ts.isCallExpression(p.expression) && ts.isIdentifier(p.expression.expression) && ['t', 'translate'].includes(p.expression.expression.text)) return true;
    p = p.parent;
  }
  return false;
}
